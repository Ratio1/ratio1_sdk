...
- dAuth job secrets (2026-07): `BaseBlockEngine.get_dauth_job_secret_bundle()` signs a job-scoped `{\"job_id\": \"...\"}` request, posts it to `/get_secrets` on the configured dAuth host, verifies the signed response, and rejects mismatched job IDs or malformed bundle shapes. The shared redaction marker is `dAuth.DAUTH_SECRET_PLACEHOLDER` (`__R1_DAUTH_SECRET__`). Request bodies, response bodies, and secret values must never be logged by this client.
...
- Summary authorization (2026-09): formatter decoding can overwrite routing fields from nested data or metadata. In `summary_discovery`, the payload handler must reject decoded NetMon messages lacking raw-envelope authorization before discovery or callbacks; decoded routing alone cannot grant trust. Tests must leave the worker filter unset so it cannot hide a missing authorization guard.
...
- 2026-08-17: MQTT publish lifecycle accounting distinguishes local publish rejection, Paho handoff, QoS callback completion, abandoned in-flight messages, and callbacks from retired client generations; none of these states proves edge application execution.
- 2026-08-17: Documented immutable heartbeat observation modes, raw reduced-mode trust boundaries, no-global-fallback behavior, and bounded callback admission status.
- 2026-09-11: MQTT readiness, SUBACK waiters, and handoff accounting belong to one active client generation; acquire the client lock before subscription/lifecycle locks, and keep Paho calls and waits outside them. A successful handoff returned after retirement is abandoned, not pending. Selected-node observation also times out before the first SUBACK, but this deadline does not bound initial synchronous connection setup (`ratio1/comm/mqtt_wrapper.py`, `ratio1/comm/heartbeat_observation.py`).
- 2026-09-11: Initial MQTT subscriptions run before the session main-loop thread starts. Transport cancellation therefore reads a sticky shutdown request, not the running flag. Early nonblocking close must still start the cleanup owner without restarting connections or waiting for observation readiness; pipeline-close transactions precede transport cancellation. Test the first constructor connection explicitly, since later reconnects can mask missing startup subscriptions (`ratio1/base/generic_session.py`, `tests/test_mqtt_session_startup_live.py`).
- 2026-09-10: Guarded summary-discovery processing against formatter routing changes after raw authorization, preserving ordinary payloads and other observation modes.
- 2026-09-14: Existing MQTT QoS configuration uses historical `int(value)` conversion followed by the `[0, 1, 2]` range check. Preserve bool/numeric-float coercion (including truncation toward zero) and integer strings such as `01`, `+1`, and `0_1`; string `true`, string `1.0`, invalid/nonfinite inputs and out-of-range results remain rejected. Cover both global/channel QoS and first-nonempty SDK environment aliases in `tests/test_mqtt_channel_qos.py`; do not apply this compatibility rule to TLS parsing or new heartbeat observation/auth validation.
